...
class POPGPC(Agent):
    def __init__(
        ...
        H: int = 3,
        HH: int = 50, # needs to be long enough for mixing
        lr_scale: Real = 0.05,
        decay: bool = False,
    ) -> None:
        """
        ...
        self.cost_fn = cost_fn #or quad_loss
        ...
        def policy_loss(M, p, w):
            """Surrogate cost function"""

            def action(state, h):
                """Action function"""
                return -self.K @ state + p + jnp.tensordot(
            ...
            final_state, _ = jax.lax.scan(evolve, np.array([[1.0],[0.0]]), np.arange(HH))
            return self.cost_fn(final_state, action(final_state, HH - 1))
    ...
    def update(self, state: jnp.ndarray, u: jnp.ndarray) -> None:
        ...
        cur_loss = self.policy_loss(self.M,self.pstar,self.noise_history)
        diff_loss = self.policy_loss(self.M,self.pstar + jnp.array([0.1]),self.noise_history)
        print(f"Loss {cur_loss}; diff_loss {diff_loss}")
        delta_M, delta_p = self.grad(self.M, self.pstar, self.noise_history)
        print(delta_p)

        lr = self.lr_scale
        lr *= (1 / (self.t + 1)) if self.decay else 1
        self.M -= lr * delta_M
        self.pstar -= lr * delta_p
        print(self.pstar)
#        self.bias -= lr * delta_bias
    ...
    def get_action(self, state: jnp.ndarray) -> jnp.ndarray:
        ...
        Returns:
            jnp.ndarray
        """
        return -self.K @ state + self.pstar + jnp.tensordot(self.M, self.last_h_noises(), axes=([0, 2], [0, 1]))
